...
CCost::CCost(std::string inContext, const json& inPayload)
{
	m_bUpdated = false;
	...
	for (auto it = inPayload.begin(); it != inPayload.end(); ++it) {
		...
		if (strcmp(it.key().c_str(), "settings") == 0)
		{
			for (auto its = it.value().begin(); its != it.value().end(); ++its)
			{
				...
				if (strcmp(its.key().c_str(), "cost1String") == 0)
				{
					buffers << its.value();
					m_sCost1 = buffers.str().substr(1, buffers.str().length() - 2);
				}
				else if (strcmp(its.key().c_str(), "cost2String") == 0)
				{
					buffers << its.value();
					m_sCost2 = buffers.str().substr(1, buffers.str().length() - 2);
				}
...
void CCost::UpdateSettings(const json& inPayload)
{
	for (auto it = inPayload.begin(); it != inPayload.end(); ++it) {
		std::stringstream buffer;
		buffer << it.key() << " : " << it.value();
